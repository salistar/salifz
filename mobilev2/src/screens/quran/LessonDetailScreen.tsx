...
export default function LessonDetailScreen({ route, navigation }: any) {
  ...
  const fetchAyahText = async (surahId: number, ayahNum: number): Promise<string> => {
    try {
      ...
      // ✅ AVANT: `آية ${ayahNum} من سورة ${surahId}`
  ...
  const generateExercises = (ayahs: AyahData[]): void => {
    ...
    allExercises.push({
      type: 'arrange_ayahs',
      ayahs,
      // ✅ AVANT: 'رتب الآيات بالترتيب الصحيح 🔤'
  ...
  const checkAnswer = (): void => {
    ...
    if (correct) {
      ...
      if (!useHeart()) {
        ...
        // ✅ AVANT: 'نفدت القلوب! ❤️', 'يمكنك شراء قلوب من المتجر'
        Alert.alert(
  ...
  const handleGoBack = (): void => {
    ...
    // ✅ AVANT: 'مغادرة الدرس؟', 'ستفقد تقدمك في هذا الدرس'
    Alert.alert(
    ...
    switch (exercise.type) {
      ...
      case 'recite_check':
        return (
          <View style={styles.exerciseContainer}>
            <Text style={styles.instruction}>{exercise.instruction}</Text>
            {/* ✅ AVANT: 'حاول قراءة الآيات من الذاكرة' */}
      ...
      {isCorrect !== null && (
        <View style={[styles.feedback, isCorrect ? styles.feedbackCorrect : styles.feedbackWrong]}>
          ...
          <Text style={styles.feedbackText}>
            {/* ✅ AVANT: 'أحسنت! +X XP' / 'حاول مرة أخرى' */}
            {isCorrect 
