...
const LEAGUES = [
  { id: 'bronze', nameKey: 'home.leagues.bronze', icon: '🥉', color: '#CD7F32', minXP: 0 },
  { id: 'silver', nameKey: 'home.leagues.silver', icon: '🥈', color: '#C0C0C0', minXP: 1000 },
  { id: 'gold', nameKey: 'home.leagues.gold', icon: '🥇', color: '#FFD700', minXP: 5000 },
  { id: 'platinum', nameKey: 'home.leagues.platinum', icon: '💎', color: '#E5E4E2', minXP: 15000 },
  { id: 'diamond', nameKey: 'home.leagues.diamond', icon: '💠', color: '#B9F2FF', minXP: 30000 },
  { id: 'master', nameKey: 'home.leagues.master', icon: '👑', color: '#9B59B6', minXP: 50000 },
];
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        ...
        {nextPrayer && (
          <TouchableOpacity onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('PrayerTimes'); }} activeOpacity={0.9}>
            <LinearGradient colors={['#667eea', '#764ba2']} style={styles.prayerCard} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
              <View style={styles.prayerHeader}>
                ...
                <TouchableOpacity style={styles.qiblaButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('Qibla'); }}>
        ...
        <View style={styles.quickActions}>
          <TouchableOpacity style={styles.actionButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('LessonsTab'); }}>
            <View style={[styles.actionIcon, { backgroundColor: '#E3F2FD' }]}><Text style={styles.actionEmoji}>📖</Text></View>
          ...
          <TouchableOpacity style={styles.actionButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('Review'); }}>
            <View style={[styles.actionIcon, { backgroundColor: '#FFF3E0' }]}><Text style={styles.actionEmoji}>🔄</Text></View>
            <Text style={styles.actionText}>{t('home.actions.review')}</Text>
          </TouchableOpacity>
          {/* ✅ NEW: Khatam */}
          <TouchableOpacity style={styles.actionButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('Khatam'); }}>
            <View style={[styles.actionIcon, { backgroundColor: '#E8F5E9' }]}><Text style={styles.actionEmoji}>📚</Text></View>
          ...
          <TouchableOpacity style={styles.actionButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('SocialTab'); }}>
            <View style={[styles.actionIcon, { backgroundColor: '#FCE4EC' }]}><Text style={styles.actionEmoji}>💬</Text></View>
        ...
        {activeKhatam && (
          <TouchableOpacity style={styles.khatamCard} onPress={() => navigation.navigate('KhatamDetail', { khatamId: activeKhatam._id })} activeOpacity={0.8}>
        ...
        <View style={styles.islamicSection}>
          ...
          <View style={styles.islamicRow}>
            <TouchableOpacity style={styles.islamicCard} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('PrayerTimes'); }}>
              <LinearGradient colors={['#667eea', '#764ba2']} style={styles.islamicGradient}>
                <Text style={styles.islamicIcon}>🕐</Text>
                <Text style={styles.islamicText}>{t('prayer.title') || 'مواقيت الصلاة'}</Text>
              </LinearGradient>
            </TouchableOpacity>
            <TouchableOpacity style={styles.islamicCard} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('Qibla'); }}>
              <LinearGradient colors={['#f093fb', '#f5576c']} style={styles.islamicGradient}>
                <Text style={styles.islamicIcon}>🧭</Text>
                <Text style={styles.islamicText}>{t('qibla.title') || 'القبلة'}</Text>
              </LinearGradient>
            </TouchableOpacity>
            <TouchableOpacity style={styles.islamicCard} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('Khatam'); }}>
        ...
        <View style={styles.socialSection}>
          ...
          <View style={styles.socialButtons}>
            <TouchableOpacity style={styles.socialButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('ConversationsList'); }}>
            ...
            <TouchableOpacity style={styles.socialButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('Halaqa'); }}>
              <View style={[styles.socialIconContainer, { backgroundColor: '#E8F5E9' }]}><Text style={styles.socialIcon}>🕌</Text></View>
            ...
            <TouchableOpacity style={styles.socialButton} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('Friends'); }}>
              <View style={[styles.socialIconContainer, { backgroundColor: '#FFF3E0' }]}><Text style={styles.socialIcon}>👥</Text></View>
        ...
        {conversations.length > 0 && (
          <View style={styles.conversationsSection}>
            <View style={styles.sectionHeader}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('ConversationsList')}><Text style={styles.seeAll}>{t('home.seeAll')}</Text></TouchableOpacity>
            </View>
            {conversations.map((conv, index) => (
              <TouchableOpacity key={conv._id || index} style={styles.conversationItem} onPress={() => navigation.navigate('Chat', { conversationId: conv._id, recipientId: conv.participants?.[0]?._id })}>
        ...
        {halaqat.length > 0 && (
          <View style={styles.halaqatSection}>
            <View style={styles.sectionHeader}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('Halaqa')}><Text style={styles.seeAll}>{t('home.seeAll')}</Text></TouchableOpacity>
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {halaqat.map((halaqa, index) => (
                <TouchableOpacity key={halaqa._id || index} style={styles.halaqaCard} onPress={() => navigation.navigate('HalaqaDetail', { halaqaId: halaqa._id })}>
              ...
              <TouchableOpacity style={[styles.halaqaCard, styles.createHalaqaCard]} onPress={() => navigation.navigate('Halaqa')}>
                <View style={[styles.halaqaIconContainer, { backgroundColor: '#E0E0E0' }]}><Text style={styles.halaqaIcon}>➕</Text></View>
        ...
        <View style={styles.goalsCard}>
          ...
          <View style={styles.goalItem}>
            ...
            <View style={styles.goalBar}><View style={[styles.goalFill, { width: `${Math.min(((dailyGoals?.xpEarned || 0) / (dailyGoals?.xpTarget || 100)) * 100, 100)}%`, backgroundColor: '#FF9800' }]} /></View>
          </View>
        </View>
        ...
        <View style={styles.challengesSection}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('Challenges')}><Text style={styles.seeAll}>{t('home.seeAll')}</Text></TouchableOpacity>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
...
const styles = StyleSheet.create({
  ...
  qiblaText: { fontSize: 12, color: '#fff', fontWeight: '600' },
  prayerName: { fontSize: 32, fontWeight: 'bold', color: '#fff', marginBottom: 12 },
  countdownRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  countdownItem: { alignItems: 'center' },
  countdownValue: { fontSize: 36, fontWeight: 'bold', color: '#fff' },
  countdownLabel: { fontSize: 12, color: 'rgba(255,255,255,0.7)' },
  countdownSeparator: { fontSize: 32, fontWeight: 'bold', color: '#fff', marginHorizontal: 12 },
  ...
  khatamInfo: { flex: 1 },
  khatamTitle: { fontSize: 18, fontWeight: 'bold', color: '#fff' },
  ...
  khatamProgressFill: { height: '100%', backgroundColor: '#fff', borderRadius: 4 },
  khatamProgressText: { fontSize: 14, fontWeight: 'bold', color: '#fff', marginLeft: 12 },
  ...
  islamicText: { fontSize: 11, fontWeight: '600', color: '#fff', textAlign: 'center' },
