/**
 * Email Verification Screen - Salifz
 */
...
export default function EmailVerificationScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    if (isSimulation && verificationType === 'otp') {
      ...
      Alert.alert('📧 رمز التحقق (للتجربة)', `رمز التحقق المرسل إلى بريدك الإلكتروني:\n\n${generatedOtp}`);
    }
    startResendTimer();
  }, []);
  ...
  const resendEmail = async () => {
    ...
    if (verificationType === 'otp') {
      ...
      Alert.alert('📧 تم الإرسال', 'تم إرسال رابط التحقق إلى بريدك الإلكتروني');
    }
    startResendTimer();
  };
    ...
    return (
      <LinearGradient colors={['#1a1a2e', '#16213e']} style={styles.container}>
        <View style={styles.successContainer}>
          ...
          <Text style={styles.successSubtitle}>جاري نقلك إلى الصفحة الرئيسية...</Text>
        </View>
      </LinearGradient>
    );
  }
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboardView}>
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.subtitle}>
            {verificationType === 'otp' ? `أدخل رمز التحقق المرسل إلى\n` : `تم إرسال رابط التحقق إلى\n`}
          ...
          <View style={styles.resendContainer}>
            {canResend ? (
              <TouchableOpacity onPress={resendEmail}><Text style={styles.resendLink}>{verificationType === 'otp' ? 'إعادة إرسال الرمز' : 'إعادة إرسال الرابط'}</Text></TouchableOpacity>
