...
export default function HalaqaChatScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (data: any) => {
      ...
      console.log('[HalaqaChat] New message received:', message._id);
    ...
    const handleConnect = () => {
      console.log('[HalaqaChat] Socket connected');
      setIsConnected(true);
      joinHalaqaChat();
    };

    const handleDisconnect = () => {
      console.log('[HalaqaChat] Socket disconnected');
  ...
  const loadMessages = async () => {
    ...
    try {
      ...
      console.error('[HalaqaChat] Load messages error:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const joinHalaqaChat = () => {
    if (halaqaId && socketService.isConnected()) {
      console.log('[HalaqaChat] Joining room:', halaqaId);
  ...
  const leaveHalaqaChat = () => {
    if (halaqaId && socketService.isConnected()) {
      console.log('[HalaqaChat] Leaving room:', halaqaId);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error('[HalaqaChat] Send error:', error);
      Alert.alert('خطأ', error?.error || error?.message || 'فشل في إرسال الرسالة');
  ...
  const handleTyping = (text: string) => {
    ...
    if (text.length > 0) {
      socketService.emit('halaqaTyping', {
        halaqaId,
        odileId: user?._id,
        username: user?.displayName || user?.username || 'عضو',
      });
  ...
  const getSenderName = (message: Message): string => {
    if (isMyMessage(message)) return 'أنت';
    return message.sender?.displayName || message.sender?.username || 'عضو';
  };
  ...
  const renderTypingIndicator = () => {
    ...
    return (
      <View style={styles.typingContainer}>
        ...
        <Text style={styles.typingText}>{names} يكتب...</Text>
      </View>
    );
  };
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyText}>{'كن أول من يبدأ المحادثة!'}</Text>
    </View>
  );
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <LinearGradient colors={[COLORS.primary, '#2E7D32']} style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <Text style={styles.headerTitle} numberOfLines={1}>{halaqaName || 'محادثة الحلقة'}</Text>
          <View style={styles.statusRow}>
            <View style={[styles.statusDot, isConnected && styles.statusDotOnline]} />
            <Text style={styles.statusText}>{isConnected ? 'متصل' : 'غير متصل'}</Text>
          </View>
        </View>
